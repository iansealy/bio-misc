...
# PODNAME: dump_ensembl_cnvs.pl
# ABSTRACT: Dump Ensembl CNVs in BED format
    ...
    @{$slices} )
{
      ...
      )
    {
        ...
        elsif ( $sv->seq_region_strand == 1 ) {
            $strand = q{+};
        }
        printf "%s\t%d\t%d\t%s\t0\t%s\n", $sv->seq_region_name,
          $sv->seq_region_start, $sv->seq_region_end, $sv->variation_name,
          $strand;
    }
}
...
dump_ensembl_cnvs.pl

Dump Ensembl CNVs in BED format
...
This script dumps all Ensembl CNVs in BED format on STDOUT.
